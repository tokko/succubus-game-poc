...
_BASE   = r"D:\claude projects\succubus-game-poc\Assets\Characters\Bakeoff\B_TRELLIS"
# TRELLIS.2 outputs a single GLB from the HF Space. We accept either name.
_PRIMARY  = os.path.join(_BASE, "succubus_b.glb")
_TRELLIS  = os.path.join(_BASE, "trellis_output.glb")

if os.path.exists(_PRIMARY):
    GLB_IN = _PRIMARY
elif os.path.exists(_TRELLIS):
    GLB_IN = _TRELLIS
else:
    raise SystemExit(f"No TRELLIS GLB found. Place the HF Space output at {_PRIMARY}")
HAS_TEXTURE = True   # TRELLIS GLB is expected to have embedded vertex colour or PBR
FBX_OUT = r"D:\claude projects\succubus-game-poc\Assets\Characters\Bakeoff\B_TRELLIS\succubus_b.fbx"
