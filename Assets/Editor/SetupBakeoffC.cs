...
/// </summary>
[InitializeOnLoad]
public static class SetupBakeoffC
{
    // ── Paths ─────────────────────────────────────────────────────────────────
    const string BASE        = "Assets/Characters/Bakeoff/C_HY3DMV";
    const string FBX         = BASE + "/succubus_c.fbx";
    const string ALBEDO      = BASE + "/succubus_pbr.jpg";
    const string METALLIC    = BASE + "/succubus_pbr_metallic.jpg";
    const string ROUGHNESS   = BASE + "/succubus_pbr_roughness.jpg";
    const string PACKED      = BASE + "/succubus_metalSmooth.png";
    const string MAT         = BASE + "/SuccubusC.mat";
    const string CTRL        = "Assets/Animations/Bakeoff/SuccubusC.controller";
    const string SCENE_PATH  = "Assets/Scenes/GameScene.unity";
    const string PREF_KEY    = "SetupBakeoffC_Done_v1";

    // ── Auto-run once ─────────────────────────────────────────────────────────
    static SetupBakeoffC()
    {
        if (EditorPrefs.GetBool(PREF_KEY, false)) return;
        EditorApplication.delayCall += Run;
    }

    [MenuItem("Tools/Setup Bake-off C (HY3D-2mv)")]
    ...
    static void Step4_CreateAnimator()
    {
        ...
        if (AssetDatabase.LoadAssetAtPath<AnimatorController>(CTRL) != null)
        {
            ...
            Debug.Log("[SetupBakeoffC] Step 4: deleted stale controller; rebuilding.");
        }
